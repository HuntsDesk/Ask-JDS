...
FOR SELECT
TO authenticated
USING ((SELECT auth.is_admin(auth.uid())));
...
-- Recreate admin policies with proper auth.is_admin() function
...
FOR INSERT
TO authenticated
WITH CHECK ((SELECT auth.is_admin(auth.uid())));
