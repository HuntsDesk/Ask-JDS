-- Production RLS Policies Creation Script (Safe Version)
-- This script recreates all RLS policies from development with public.is_admin calls
-- Uses IF NOT EXISTS where possible and drops existing policies when needed
...
DROP POLICY IF EXISTS "Anyone can read flashcard collections" ON public.collections;
DROP POLICY IF EXISTS "Users can create flashcard collections" ON public.collections;
DROP POLICY IF EXISTS "Users can delete non-official flashcard collections" ON public.collections;
DROP POLICY IF EXISTS "Users can update non-official flashcard collections" ON public.collections;
...
-- Drop existing course enrollments policies
...
-- Drop existing error logs policies
...
-- Drop existing flashcards policies
...
-- Drop existing message counts policies
DROP POLICY IF EXISTS "Users can insert their own message counts" ON public.message_counts;
DROP POLICY IF EXISTS "Users can update their own message counts" ON public.message_counts;
DROP POLICY IF EXISTS "Users can view their own message counts" ON public.message_counts;
...
-- Drop existing models policies
DROP POLICY IF EXISTS "models_access" ON public.models;
...
-- Drop existing query logs policies
DROP POLICY IF EXISTS "Only admins can view query logs" ON public.query_logs;
...
-- Drop existing system prompts policies
...
-- Drop existing threads policies
...
-- Drop existing user entitlements policies
...
-- Drop existing user subscriptions policies
