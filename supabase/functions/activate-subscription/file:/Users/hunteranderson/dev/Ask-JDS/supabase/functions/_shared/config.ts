 ...
 */ export const getConfig = ()=>{
  ...
  const stripeSecretKey = isProduction ? Deno.env.get('STRIPE_LIVE_SECRET_KEY') || '' : Deno.env.get('STRIPE_SECRET_KEY') || '';
  const stripeWebhookSecret = isProduction ? Deno.env.get('STRIPE_LIVE_WEBHOOK_SECRET') || '' : Deno.env.get('STRIPE_TEST_WEBHOOK_SECRET') || '';
  const publicAppUrl = Deno.env.get('PUBLIC_APP_URL') || '';
  // Model configuration with obfuscated names
  const aiPrimaryChatModel = isProduction ? Deno.env.get('AI_MODEL_PRIMARY_PROD') || 'jds-titan' : Deno.env.get('AI_MODEL_PRIMARY_DEV') || 'jds-titan';
  const aiSecondaryTitleModel = isProduction ? Deno.env.get('AI_MODEL_SECONDARY_PROD') || 'jds-flash' : Deno.env.get('AI_MODEL_SECONDARY_DEV') || 'jds-flash';
 ...
 */ const _getActualModelName = (codeName)=>{
  const modelMap = {
 ...
 */ export const getModelEndpoint = (modelCodeName)=>{
