-- URGENT: Add User Ownership to Subjects Table
...
-- For existing non-official subjects, we need to decide:
-- Option A: Set to NULL (admin will need to reassign)
-- Option B: Set to first admin user if available
-- Going with Option A for safety
...
-- ADMIN: Admins can manage all subjects
