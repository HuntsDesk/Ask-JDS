-- Phase 3.2 FIXED: Moderate Policy Consolidation
...
-- FIXED: Uses correct relationship-based user ownership checks

BEGIN;

-- =============================================================================
-- MODERATE POLICY CONSOLIDATIONS - CORRECTED
-- =============================================================================
...
-- FIXED: User ownership checked via collections.user_id relationship
...
USING (
  ...
  OR 
  -- Users can delete their own collection_subjects (via collections ownership)
  (EXISTS (
...
WITH CHECK (
  ...
  ((select auth.is_admin()) = true)
  OR 
  -- Users can insert their own collection_subjects (via collections ownership)
  (EXISTS (
    SELECT 1 FROM collections c 
    WHERE c.id = collection_subjects.collection_id 
    AND c.user_id = (select auth.uid())
...
-- These are identical - consolidate to single policy
...
USING (
  ...
  ((select auth.is_admin()) = true)
  OR 
  -- Users can delete their own junction records (via collections ownership)
  (EXISTS (
    SELECT 1 FROM collections c 
...
USING (
  ...
  OR
  -- Users can view their own flashcards (created_by column)
...
-- Consolidate to single policy with created_by column
...
USING (
  ...
  ((select auth.is_admin()) = true)
  OR 
  -- Users can update their own flashcards (created_by column)
  ((select auth.uid()) = created_by)
)
WITH CHECK (
